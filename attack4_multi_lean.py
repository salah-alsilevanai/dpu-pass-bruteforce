...
def worker(phone_no, password, lock, success_flag):
    # Skip if we already found a working password
    ...
    with lock:
        if success:
            # Set the flag to stop other workers
            success_flag.set()
...
def process_single_number(phone_number, lock):
    ...
    try:
        ...
        # Create a success flag for this phone number
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for pwd in passwords:
                future = executor.submit(worker, phone_number, pwd, lock, success_flag)
                futures.append(future)
            
            # Wait for either success or all attempts to complete
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if success_flag.is_set():
                        ...
                        for f in futures:
                            f.cancel()
                        print(f"Found working password for {phone_number}, stopping other attempts...")
                        break
